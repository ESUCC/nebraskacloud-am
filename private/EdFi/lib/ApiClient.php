...
class ApiClient
{
     ...
     * @return mixed
     */
    public function callApi($resourcePath, $method, $queryParams, $postData, $headerParams, $responseType = null, $includeBasePath = true)
    {
    ...
    */
    protected function http_parse_headers($raw_headers)
    {
     ...
     public function populateAuthToken($clientId, $clientSecret){

    	$oAuthUrl = $this->config->getOAuthUrl() . "/admin/oauth/authorize";
		$codeRaw = $this->callApi($oAuthUrl, 'GET', array(
			'Client_id' => $clientId,
			'Response_type' => 'code',
		), array(), array(), null, false);

		if(!isset($codeRaw[0]->Code)){
			throw new \Exception('Invalid Response');
		}
		$code = $codeRaw[0]->Code;
		$oAuthUrl = $this->config->getOAuthUrl() . "/admin/oauth/token";
		$access_tokenRaw = $this->callApi($oAuthUrl, 'GET', array(
			'Client_id' => $clientId,
		...
		if(!isset($access_tokenRaw[0]->Access_token)){
			throw new \Exception('Invalid Response');
		}
		$access_token = $access_tokenRaw[0]->Access_token;
		$this->config->setAccessToken($access_token);
		$this->config->addDefaultHeader('Authorization', "Bearer {$access_token}");
    }
}
